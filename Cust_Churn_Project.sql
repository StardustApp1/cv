...
--Gender Bifurcation
Select Gender                                                                    --Gender Column
	, Count(Gender) AS [TotalCount]                                              --Gender Count
	, Count(Gender) * 100.0 / (Select Count(*) From stg_Churn) AS _Percentage    --Gender %age 
From stg_Churn
Group by Gender


--Contract Bifurcation
Select Contract                                                                    --Contract Column
	, Count(Contract) AS [TotalCount]                                              --Contract Count
	, Count(Contract) * 100.0 / (Select Count(*) From stg_Churn) AS _Percentage    --Contract %age 
From stg_Churn
Group by Contract


--Customer Status Bifurcation
...
SELECT 
    SUM(CASE WHEN Customer_ID IS NULL THEN 1 
		ELSE 0 END) AS Customer_ID_Null_Count,
...
--Making duplicate reduces the performance.
...
--"TenureGrpSorting"(Custom Column)
